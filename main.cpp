...
void udp_server()
{
    ...
    auto shared_stream = make_stream(ep);
    DataInputStream in(shared_stream);
    DataOutputStream out(shared_stream);
...
int main()
{
    // TCP test
    {
        ...
        auto tcp_stream = make_stream(ep);

        DataOutputStream out(tcp_stream);
        DataInputStream  in(tcp_stream);
    ...
    // UDP test
    {
        ...
        auto udp_stream = make_stream(client_ep);

        DataOutputStream out(udp_stream);
        DataInputStream  in(udp_stream);
    ...
    // File test: write and read a string (unbuffered)
    {
        ...
        DataOutputStream dout(std::move(fsink));
        dout.write_string("Hello File!");
        dout.flush();

        FileInputStream fsrc("test.bin");
        DataInputStream din(std::move(fsrc));
