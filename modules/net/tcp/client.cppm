...
export namespace modern::net {
  ...
  // Ignore errors; never propagate exceptions
    ...
    {
      if (!ep_)
        throw net_io::ConnectionException("Connection failed: no endpoint set", 0);
      ...
      if (fd_ == invalid_socket)
      {
        ...
        throw net_io::NetworkException("Failed to create socket", errno);
      }
          ...
          ) < 0)
      {
        ...
        throw net_io::ConnectionException("Failed to connect", err, endpoint_str);
      }
    }
    ...
    void write(const char* data, std::size_t size)
    {
      if (fd_ == invalid_socket)
      {
        ...
        throw net_io::NetworkException("Write failed: socket not open", 0);
      }
#if defined(_WIN32)
      ...
      if (ret < 0 || static_cast<std::size_t>(ret) != size)
        throw net_io::NetworkException("Write failed", errno);
#endif
    }
  ...
  static_assert(net_io_concepts::Transportable<modern::net::TcpClient>, "TcpClient does not implement Transportable concept!");
}

export namespace net_io {

using TcpClient = modern::net::TcpClient;
