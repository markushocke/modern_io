...
/**
 * @brief Plattformunabhängiger synchroner IPC-Transport (Pipe/FIFO/NamedPipe)
 */
...
public:
    ...
    explicit IPCTransport(const std::string& name, bool write = false)
    {
        ...
        if (fd_ < 0)
            throw net_io::SocketException(std::string("IPCTransport open failed: ") + std::strerror(errno), errno);
    }
    ...
    std::size_t read(char* buf, std::size_t n)
    {
        ...
        if (ret < 0)
            throw net_io::SocketException(std::string("IPCTransport read failed: ") + std::strerror(errno), errno);
    ...
    void write(const char* buf, std::size_t n)
    {
        ...
        if (ret < 0)
            throw net_io::SocketException(std::string("IPCTransport write failed: ") + std::strerror(errno), errno);
    }
...
private:
    ...
    explicit IPCTransport(const std::string& name, bool write = false)
    {
        ...
        if (h_ == INVALID_HANDLE_VALUE)
            throw net_io::SocketException("IPCTransport open failed", -1);
    }
    ...
    std::size_t read(char* buf, std::size_t n)
    {
        ...
        if (!ReadFile(h_, buf, static_cast<DWORD>(n), &read, nullptr))
            throw net_io::SocketException("IPCTransport read failed", -1);
    ...
    void write(const char* buf, std::size_t n)
    {
        ...
        if (!WriteFile(h_, buf, static_cast<DWORD>(n), &written, nullptr))
            throw net_io::SocketException("IPCTransport write failed", -1);
    }
...
static_assert(net_io_concepts::Readable<net_io::IPCTransport> && net_io_concepts::Writable<net_io::IPCTransport>,
