...
export namespace modern::net {
  ...
  public:
    ...
    void start()
    {
...
#endif
          throw net_io::ResolutionException(msg, "0.0.0.0");
        }
      ...
      if (listen_fds_.empty())
        throw net_io::BindException("Failed to bind to any address", errno, "0.0.0.0", endpoint_.port);
    }
    ...
    TcpClient accept()
    {
      while (true)
      {
        ...
        if (ready < 0)
          throw net_io::NetworkException("select failed", errno);

        if (ready == 0 && accept_timeout_ms_ >= 0)
          throw net_io::TimeoutException("Accept timeout");
  ...
  static_assert(net_io_concepts::Acceptable<modern::net::TcpServer>, "TcpServer does not implement Acceptable concept!");
}

export namespace net_io {

using TcpServer = modern::net::TcpServer;
